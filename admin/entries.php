...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($report_id_to_update) { // LÓGICA DE ATUALIZAÇÃO
            ...
            // Usar GET para a mensagem de sucesso para evitar reenvio do formulário
...
if ($church_id) {
    ...
    $stmt_fetch = $conn->prepare("SELECT *, created_at as entry_timestamp FROM service_reports WHERE church_id = ? ORDER BY created_at DESC, id DESC LIMIT 20");
        ...
        <div class="flex-1 flex flex-col w-full min-w-0">
            ...
            <main class="flex-1 p-4 sm:p-6 overflow-y-auto">
                ...
                <div class="bg-white rounded-lg shadow p-4 sm:p-6 mt-8">
                    <h2 class="text-lg font-semibold text-gray-800 mb-4">Últimos Relatórios Registados</h2>
                            ...
                            <table class="w-full text-sm text-left text-gray-500">
                                ...
                                <tbody>
                                    <?php if (empty($reports)): ?>
                                        <tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">Nenhum relatório registado ainda.</td></tr>
                                    <?php else: ?>
                                        <?php foreach ($reports as $report): ?>
                                            <tr class="bg-white border-b hover:bg-gray-50">
                                                <td class="px-4 py-4 font-medium text-gray-900 whitespace-nowrap"><?php echo date('d/m/Y H:i', strtotime($report['entry_timestamp'])); ?></td>
                                                ...
                                                <td class="px-4 py-4 font-medium text-green-600"><?php echo number_format($report['total_offering'], 2, ',', '.'); ?></td>
