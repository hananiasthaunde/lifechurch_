...
// Apenas pastores e administradores podem aprovar membros
$can_approve = in_array($user_role, ['pastor', 'pastor_principal', 'master_admin']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $can_approve) {
    if (isset($_POST['approve_member'])) {
        $member_to_approve_id = (int)$_POST['member_id'];
        $stmt = $conn->prepare("UPDATE users SET is_approved = 1 WHERE id = ? AND church_id = ?");
        $stmt->bind_param("ii", $member_to_approve_id, $church_id);
        if ($stmt->execute()) {
        ...
        $member_to_reject_id = (int)$_POST['member_id'];
        // Usar transação para garantir que ambas as exclusões ocorram
        $conn->begin_transaction();
        try {
            // Apaga da tabela 'members' se existir
            $stmt_members = $conn->prepare("DELETE FROM members WHERE user_id = ? AND church_id = ?");
            $stmt_members->bind_param("ii", $member_to_reject_id, $church_id);
            $stmt_members->execute();

            // Apaga da tabela 'users'
            $stmt_users = $conn->prepare("DELETE FROM users WHERE id = ? AND church_id = ?");
            $stmt_users->bind_param("ii", $member_to_reject_id, $church_id);
            $stmt_users->execute();
            ...
            $conn->rollback();
            $error = "Erro ao rejeitar o membro.";
        }
    }
}
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if ($result_check->num_rows > 0) {
        ...
        try {
            $stmt_members = $conn->prepare("DELETE FROM members WHERE user_id = ? AND church_id = ?");
            $stmt_members->bind_param("ii", $member_to_delete_id, $church_id);
            $stmt_members->execute();
    ...
    if ($name && $email && $role) {
        ...
        elseif ($action === 'add_member') {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    $user_new_id = $conn->insert_id;
                    $stmt_member = $conn->prepare("INSERT INTO members (user_id, church_id) VALUES (?, ?)");
                    $stmt_member->bind_param("ii", $user_new_id, $church_id);
                    $stmt_member->execute();
...
$members = [];
if ($church_id) {
    // Query corrigida para buscar todos os usuários da igreja, e não apenas da tabela 'members'
                                        ...
                                        <tr class="bg-white border-b hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex items-center space-x-1">
                                                    <?php if ($can_approve && !$member['is_approved']): ?>
