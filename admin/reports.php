        ...
        <header class="bg-white border-b border-gray-200 shadow-sm z-20">
            <div class="flex items-center justify-between h-16 px-6">
                ...
                <div class="relative">
                    ...
                    <div id="user-menu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-20"><a href="settings.php" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"><i class="ri-settings-3-line mr-3"></i>Definições</a><a href="logout.php" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"><i class="ri-logout-box-line ri-lg mr-3"></i>Sair</a></div>
                </div>
            </div>
        </header>
            ...
            if(dayCell && dayCell.dataset.date) {
                ...
                if (dayData) {
                    ...
                    if (dayData.expenses && dayData.expenses.length > 0) {
                        ...
                        detailsHTML += `<div><h4 class="font-semibold text-red-700 mb-2 mt-4">Saídas</h4><ul class="space-y-2">`;
                        dayData.expenses.forEach(item => detailsHTML += `<li class="text-sm flex justify-between"><span>${item.description} (${item.category || 'N/A'})</span><span class="font-medium">${formatCurrency(item.amount)}</span></li>`);
                        detailsHTML += `</ul></div>`;
                    }
                }
